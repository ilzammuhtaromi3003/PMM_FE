      ...
      <div v-else class="container-fluid bg-secondary-subtle py-5">
        <div class="container">
          ...
          <div class="col-lg-8 col-md-8 col-sm-8 mx-auto">
            <h2 class="text-center px-5 fs-5" id="description">Menyajikan kenyamanan hunian komersial dan bersubsidi dalam cluster dengan one gate way dan keamanan 24 jam.</h2>
          </div>
          <h3 class="m-5" id="element"></h3>
          <!-- Judul "Cluster" -->
          <h1 class="text-center my-4 fs-3">Cluster</h1>
  
          <!-- Grid column -->
          <div class="row justify-content-center">
            <div v-for="development in residentialDevelopments" :key="development.id" class="col-md-6 col-lg-4 my-2">
              <div class="position-relative ratio ratio-1x1">
                ...
                <nuxt-link :to="`/development/residential/${development.slug}`" class="image-link"></nuxt-link>
                <!-- Tautan Nuxt -->
    ...
    methods: {
      fetchDevelopments() {
        ...
        fetch(`${this.baseUrl}/api/developments`)
          ...
          .then(data => {
            console.log('Developments Fetched:', data)  // Debugging
            const activeDevelopments = data.developments
              .filter(development => development.dev_category === 'residential' && development.is_active === 1);
            ...
            this.residentialDevelopments = [...availableItems, ...comingSoonItems, ...soldItems];
  
            const firstDevelopment = this.residentialDevelopments.length > 0 ? this.residentialDevelopments[0] : {};
      ...
      navigateToDevelopment(developmentSlug) {
        console.log('Navigating to Development:', developmentSlug)  // Debugging
        this.$router.push(`/development/residential/${developmentSlug}`);
      },
      async fetchData() {
        try {
          const response = await axios.get(`${this.baseUrl}/api/setting/12`);
