<template>
    <div class="container-fluid bg-secondary-subtle p-5">
      ...
      <div v-else class="container">
        <!-- Judul "Commercial" -->
        <h1 class="text-center fs-1 py-2">Access</h1>
  
        <!-- Grid column -->
        <div class="row justify-content-center py-5">
          <!-- Loop through accesses and display them -->
          <div v-for="access in accesses" :key="access.id" class="col-lg-6 col-md-6 col-sm-6  text-center p-2">
            <nuxt-link :to="`/access/${slugify(access.access_title)}`" class="position-relative text-decoration-none">
              <div class="position-relative ratio ratio-1x1">
                <!-- Menggunakan access_image sebagai gambar -->
                <img :src="getImageUrl(access.access_image)" class="img-fluid img-crop-center"
                  :alt="access.access_title">
                <div class="container button-overlay">
                  <!-- Navigasi ke accessDetail saat tombol ditekan -->
                  <span class="text-button btn btn-primary fs-6 border-2 border-white text-white image-button"
                    id="image-button">{{ access.access_title }}</span>
                </div>
              </div>
            </nuxt-link>
          </div>
  ...
  export default {
    ...
    async created() {
      ...
      try {
        // Mengambil data akses dari API
        const response = await axios.get(`${this.baseUrl}/api/access`);
        this.accesses = response.data.accesses;
      } catch (error) {
        console.error(error);
      } finally {
    ...
    async fetchData() {
      try {
        const response = await axios.get(`${this.baseUrl}/api/setting/15`);
